...
public class CarDAO extends HttpServlet {
	...
	public ArrayList<Car> getSearch(String model_search) {
		ArrayList<Car> list = new ArrayList<Car>();
		String SQL = null;
	      try {
	    	  	if(model_search!=null & !model_search.equals("")) {
	    	  		SQL = "select * from car where car_model like N'%" + model_search.trim() + "%' and ROWNUM <= 12 order by LIST_NUM desc";
	    	  	}
	            PreparedStatement pstmt=conn.prepareStatement(SQL);
	...
	public ArrayList<Car> getEasy_Search1(String manufac, String model, String detail) {
		ArrayList<Car> list = new ArrayList<Car>();
		String SQL = null;
	      try {
	    	  	if(manufac!=null & !manufac.equals("") & model!=null & !model.equals("") & detail!=null & !detail.equals("")) {
	    	  		SQL = "select * from car where manufac like N'%" + manufac.trim() + "%' and car_model like N'%" + model.trim() + "%' and car_detail_model like N'%" + detail.trim() + "%' and ROWNUM <= 12 order by LIST_NUM desc";
	    	  	}
	            PreparedStatement pstmt=conn.prepareStatement(SQL);
				rs=pstmt.executeQuery();//select
			while(rs.next()) {
	...
	public ArrayList<Car> getEasy_Search2(String manufac, int low_price, int high_price) {
		ArrayList<Car> list = new ArrayList<Car>();
		String SQL = null;
	      try {
	    	  	if(manufac!=null & !manufac.equals("")) {
	    	  		System.out.println("SQL문을 실행합니다....");
	    	  		SQL = "select * from car where manufac like N'%" + manufac.trim() + "%' and price >= " + low_price + "and price <=" + high_price + "and ROWNUM <= 12 order by LIST_NUM desc";
	    	  	}else {
	    	  		System.out.print("SQL문을 실행할 수 없습니다....");
	    	  	}
	...
	public ArrayList<Car> getEasy_Search3(String model, int low_year, int high_year) {
		ArrayList<Car> list = new ArrayList<Car>();
		String SQL = null;
	      try {
	    	  	if(model!=null & !model.equals("")) {
	    	  		SQL = "select * from car where car_kind like N'%" + model.trim() + "%' and car_year >= " + low_year + "and car_year <=" + high_year + "and ROWNUM <= 12 order by LIST_NUM desc";
	    	  	}
	            PreparedStatement pstmt=conn.prepareStatement(SQL);
				rs=pstmt.executeQuery();//select
			while(rs.next()) {
